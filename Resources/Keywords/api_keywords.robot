...
${API_ENDPOINT_EDIT_PRODUCT}          v1/products/00038d93-e998-4be8-92f9-e28d4f8efc48?
${API_ENDPOINT_CREATE_PRODUCT}        v1/products
...
${BODY_CREATE_PRODUCT}                {"title": "Robot Framework Data",    "external_id": "1ec814ddc276a05d5bf30a1b167e0912",    "season": "AI20",    "tags": "AI20, Season: AI20, harrison, Supplier: harrison, feed-cl0-Harrison, WOMAN, size-type-EURO, COLOR: BLACK, feed-color-black, ProductID: DMSJADONBK15265001JADONBLACK, Tmall, GoodsNameTitle, NewTmall",    "category": "Shoes",    "product_type": "HARRISON > WOMAN > SHOES > FLAT SHOES",    "brand": "DR. MARTENS",    "description": "Robot Framework Data description",    "supplier_name": "baltini",    "gender": "WOMAN",    "product_code": "DMSJADONBK15265001JADONBLACK",    "color": "BLACK",    "product_images": [        "https://t.ly/SDB6W", "https://t.ly/SDB6W", "https://t.ly/SDB6W","https://t.ly/SDB6W"    ],    "variants": [        {            "retailer_name": "Harrison",            "retailer_code": "Harrison",            "retailer_currency": "EUR",            "external_id": "db_erp_harrison:::5190249:::39",            "title": "39",            "barcode": "2117548645308",            "sku": "2117548645308",            "quantity": 0,            "cost_price_orig_ccy": 158.01,            "market_price_orig_ccy": 229.0,            "cost_price_ccy": "EUR",            "cost_price_usd": 158.01,            "market_price_usd": 225,            "compare_at_price_usd": 337,            "weight": 0,            "size_checking_status": "done"        },        {            "retailer_name": "Harrisan",            "retailer_code": "Harrisan",            "retailer_currency": "EUR",            "external_id": "db_erp_harrison:::5190249:::40",            "title": "40",            "barcode": "2117548645308",            "sku": "2117548645308",            "quantity": 10,            "cost_price_orig_ccy": 1587.01,            "market_price_orig_ccy": 2297.0,            "cost_price_ccy": "EUR",            "cost_price_usd": 158.017,            "market_price_usd": 2257,            "compare_at_price_usd": 3377,            "weight": 0        }    ]}
...
Set Product Detail data back to original
    ...
    ${FULL_URL}=    Catenate    ${BASE_URL}    ${API_ENDPOINT_EDIT_PRODUCT}

    # Execute the PUT request
    Create Session    edit_product    ${BASE_URL}    headers=${HEADERS}
    ${response}=    PUT On Session    edit_product    ${API_ENDPOINT_EDIT_PRODUCT}    data=${BODY}
...
Create a Product Testing
    # Set headers including the authentication token
    ${ACCESS_TOKEN}=    Perform Login using API
    &{HEADERS}=    Create Dictionary    Content-Type=application/json    Authorization=Bearer ${ACCESS_TOKEN}

    # Define the raw body (JSON payload) to send with the PUT request
    ${BODY}=    Set Variable    ${BODY_CREATE_PRODUCT}

    # Concatenate the base URL and endpoint
    ${FULL_URL}=    Catenate    ${BASE_URL}    ${API_ENDPOINT_CREATE_PRODUCT}

    # Execute the PUT request
    Create Session    create_product    ${BASE_URL}    headers=${HEADERS}
    ${response}=    POST On Session    create_product    ${API_ENDPOINT_CREATE_PRODUCT}    data=${BODY}

    # Log response details
    ${json_obj}=    Evaluate    json.loads('''${response.content}''')    json
    ${id}=    Set Variable    ${json_obj["data"]["id"]}
    Log To Console    ${id}
    # Log    ${response.status_code}
    # Log    ${response.content}
